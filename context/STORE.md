...
- **Noise / decoys:** Generated by `build_bundle()` from a **document-specific corruption plan** (`corruption_plan.py`) applied to the canonical parse (`missing_clause`, `altered_text`, `extra_clause`, `reordered`). Plans extract money, duration, obligation, and other spans from each filing; optional Gemini mode for less predictable edits. Cached under `legalDocs/corruption_plans/`.
...
| Decoy bundle generator | done      | `docProcessing/bundle.py` + `corruption_plan.py` — document-specific, seeded decoys |
...
| 2026-08-12 | Document-specific corruption plans | corruption_plan.py — per-document spans; local + optional Gemini; cached plans |
| 2026-08-12 | Portfolio cross-type strategy | parallel_cross_type_discrimination — five specialists on one shared prompt |
| 2026-08-12 | Portfolio max_concurrent = 5 | Caps parallel Gemini calls during portfolio runs; reduces free-tier rate limits |
| 2026-08-12 | RUN_DELAY_SEC = 20 between matrix runs | Experiment runner throttles sequential runs after rate-limit failures |
| 2026-08-12 | Test suite green (92 pass, 1 skip) | Full pytest after portfolio + corruption plan work |
