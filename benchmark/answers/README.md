...
Answers are derived from the **canonical parse** of each signed MSA (`fixtures/contracts/public/{document_id}.txt`). They do not reference decoys and do not change between runs.
...
|-------------|----------------|-------------------|
| `edgar_edgemode_inc_ex10.1` | ICC arbitration (pass), mutual indemnity (fail) | Fee indexation % |
...
1. Parse the fixture and locate supporting clauses: `build_bundle_from_file(fixtures/contracts/public/{document_id}.txt)`.
